...
class PipeVisualComponent(VisualComponent, brica1.PipeComponent):
    def __init__(self, dim):
        super(PipeVisualComponent, self).__init__()
        self.set_map("in", "out")
        self.set_map("token_in", "token_out")

class ModeledVisualComponent(VisualComponent):
...
class MotorComponent(brica1.Component):
    ...
    def __init__(self, in_dim, n_action, rl, train):
        ...
        if rl:
            ...
            max_episode_timesteps=train["episode_count"]*train["max_steps"], n_action=n_action, obs_dim=in_dim, parent=self)
            self.env.reset()
    ...
    def fire(self):
        if self.rl:
            ...
            if self.init:
                    ...
                    if state.dtype == np.dtype('int16'):
                        state = np.zeros(self.in_dim, dtype='float64')
                    self.act(state)
                    self.init = False
        else:
...
class CognitiveArchitecture(brica1.Module):
    def __init__(self, observation_dim, motor_obs_dim, n_action, rl, train, modelp):
        ...
        if modelp:
            ...
            self.visual = PipeVisualComponent(observation_dim)
